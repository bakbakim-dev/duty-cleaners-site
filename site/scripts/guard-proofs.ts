...
export const GUARD_PROOFS: GuardProof[] = [
  { guard: "src/lib/booking-handoff.test.ts", target: "supabase/functions/booking-handoff/handler.ts", find: "if (!allowed) return reply(403, { error: \"origin\" });", replace: "/* origin check removed */", failing: "limits endpoint origins, actions, body size and traffic without granting booking access", why: "Accepts a preflight from an unapproved origin." },
  ...
  { guard: "src/lib/booking-handoff.test.ts", target: "src/lib/booking-handoff.ts", find: "if (!response.ok) return null;", replace: "if (true) return null;", failing: "uses a bounded encrypted handoff and never falls back to a personal-data URL", why: "Rejects successful sealing responses." },
  { guard: "src/lib/booking-handoff.test.ts", target: "supabase/functions/booking-handoff/crypto.ts", find: "payload.expires <= now", replace: "payload.expires < now - TTL_MS", failing: "round trips without plaintext and rejects tampering, expiry, wrong keys and foreign fields", why: "Accepts an expired personal-data capability." },
