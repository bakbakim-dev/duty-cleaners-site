...
export const GUARD_PROOFS: GuardProof[] = [
  ...
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "hash_hmac('sha256', 'resume:' . $requestId, $config['encryption_key'], true)", replace: "random_bytes(32)", failing: "gives a confirmed quote a short own-domain link, the same one on every retry", why: "A retried delivery writes a second link file and puts a different link on the contact." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: " || str_contains((string) ($payload['source'] ?? ''), '(call-back requested)')) return '';", replace: ") return '';", failing: "never gives a call-back or an unconfirmed lead a link, and an older page the bare booking page", why: "A call-back request gets a booking link it never asked for." },
  ...
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "dc_ghl_shown_fields($config, $session));",
    replace: "[]);",
