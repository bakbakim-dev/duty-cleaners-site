...
export const GUARD_PROOFS: GuardProof[] = [
  ...
  { guard: "src/lib/ghl-lock-prune-php.test.ts", target: "public/api/ghl-quote.php", find: "'*.json.lock'", replace: "'*.json.keep'", failing: "removes the locks of settled leads and keeps every lock that could still be used", why: "Every lead keeps its lock file forever and the hosting plan's file quota fills up." },
  { guard: "src/lib/ghl-lock-prune-php.test.ts", target: "public/api/ghl-quote.php", find: "$locks = dc_ghl_prune_locks($config);", replace: "null;", failing: "runs from the cron retry", why: "The clean-up exists but never runs." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "|coupon|gclid|", replace: "|coupon|f_name|email|phone|dc_notes|gclid|", failing: "keeps only the booking page's public keys in the saved selections", why: "Stores a customer's contact details and notes in the reusable booking selections." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "hash_hmac('sha256', 'resume:' . $key, $config['encryption_key'], true)", replace: "random_bytes(32)", failing: "gives a confirmed quote a short own-domain link, the same one on every retry", why: "A retried delivery writes a second link file and puts a different link on the contact." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "    if (str_contains((string) ($payload['source'] ?? ''), '(call-back requested)')) return '';\n", replace: "", failing: "never gives a call-back a link, and resets a new price check or an older page to the bare booking page", why: "A call-back request gets a booking link it never asked for." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "if ($linkFieldId !== null) $customFields[] = ['id' => $linkFieldId, 'field_value' => $link];", replace: "// link not written", failing: "writes the link on the contact of a confirmed quote", why: "The text message never receives the customer's link." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "    if (($payload['stage'] ?? '') !== 'confirm' || $query === '') return DC_GHL_BOOKING_ORIGIN . '/booknow';", replace: "    if ($query === '') return DC_GHL_BOOKING_ORIGIN . '/booknow';", failing: "never gives a call-back a link, and resets a new price check or an older page to the bare booking page", why: "A new price check keeps an earlier quote's link, so the follow-ups send the wrong quote." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "        $shown['booking_query'] = dc_ghl_booking_query($value['booking_query'] ?? null);", replace: "        $shown['booking_query'] = (string) ($value['booking_query'] ?? '');", failing: "gives a visitor who left the price screen a link to the quote they last saw", why: "A presence report could store private fields in the saved booking selections." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "        if ($linkFieldId !== null) $extra[] = ['id' => $linkFieldId, 'field_value' => $link];", replace: "", failing: "gives a visitor who left the price screen a link to the quote they last saw", why: "A visitor who leaves gets follow-ups with no link back to their quote." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "src/components/quote/QuoteFlow.tsx", find: 'booking_query: bookingQuery === null ? "" : splitBookingQuery(bookingQuery).publicQuery,', replace: 'booking_query: bookingQuery ?? "",', failing: "sends the booking page's public selections with the confirmed quote, never the private fields", why: "The presence report sends entry codes and notes to the relay." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/resume.php", find: "$url .= '#dc_handoff=' . dc_seal(", replace: "$url .= '#x=' . dc_seal(", failing: "opens the quote on the booking page with only the name, email and phone sealed in", why: "The booking page opens without the customer's name, email and phone." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "(int) ($record['expires_at'] ?? 0) <= time()", replace: "false", failing: "sends an unknown or expired code to the plain booking page, never an error", why: "An expired link keeps handing out the customer's details." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "if ($expires < $cutoff && (int) @filemtime($path) < $cutoff && @unlink($path)) $removed++;", replace: "if (false) $removed++;", failing: "the cron job removes link files two days after they expire", why: "Every confirmed quote leaves a file behind for good on a plan that counts files." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/api/ghl-quote.php", find: "$links = dc_ghl_prune_resume($config);", replace: "$links = 0;", failing: "the cron job removes link files two days after they expire", why: "The link clean-up exists but never runs." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "public/.htaccess", find: "  RewriteRule ^r/([A-Za-z0-9]{10})/?$ /api/resume.php?c=$1 [L]\n", replace: "", failing: "routes /r/<code> to resume.php and points at the same booking host as the funnel", why: "Every link in a text message lands on the 404 page." },
  { guard: "src/lib/booking-resume-php.test.ts", target: "src/components/quote/QuoteFlow.tsx", find: "booking_query: splitBookingQuery(bookingQuery).publicQuery,", replace: "booking_query: bookingQuery,", failing: "sends the booking page's public selections with the confirmed quote, never the private fields", why: "Sends the entry codes and notes to the relay inside the booking selections." },
  ...
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "if ((edited[key] && !(key === 'dc_addr' && state.addressSelectionDone)) ||", replace: "if (false &&", failing: "preserves existing data and never overwrites a customer's later edit", why: "Overwrites a customer's revised booking-form value." },
  ...
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "heading.textContent = 'Here’s your price — pick a date to lock it in';", replace: "heading.textContent = heading.textContent;", failing: "tells only a visitor whose details arrived that they carried over", why: "Leaves the neutral booking-page heading in place after a quote handoff." },
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "function addressHeading() { return headingMatching(/^address details\\b/i); }", replace: "function addressHeading() { return headingMatching(/^address details:/i); }", failing: "on desktop shows the whole Address Details heading after the arrival time, and Book My Clean too on a tall screen", why: "Misses the live heading (it carries a subtitle), so the jump lands on the address box and cuts the heading off." },
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "if (guideKeepingBook(paymentTarget, paymentRoom, addressHeading(), 24)) paymentGuideDone = true;", replace: "if (guideTo(paymentTarget)) paymentGuideDone = true;", failing: "on desktop keeps Book My Clean visible after the address and shows as much of Address Details as fits", why: "Restores the heading-at-top payment jump that scrolls Address Details away." },
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "document.querySelectorAll('.tjs-summary-mob,.tjs-cookie-fixed')", replace: "document.querySelectorAll('.tjs-summary-mob')", failing: "keeps the button above BookingKoala's cookie notice", why: "Lets the cookie notice cover Book My Clean." },
  { guard: "src/lib/booking-prefill.test.ts", target: "../bk-prefill-v2.js", find: "document.querySelectorAll('.tjs-summary-mob,.tjs-cookie-fixed')", replace: "document.querySelectorAll('.tjs-cookie-fixed')", failing: "on a phone puts each heading near the top and lands Book My Clean above the summary bar", why: "Lets the phone summary bar cover Book My Clean." },
  {
  ...
  },
  {
    ...
    find: "rows.filter((r) => withGst(dollars(r)) <= amount).at(-1) ?? null;",
    replace: "rows.filter((r) => withGst(dollars(r)) <= amount).at(0) ?? null;",
  ...
  {
    guard: "src/lib/commercial-context.test.ts",
    target: "src/lib/commercial-context.ts",
    find: 'path === "/commercial-cleaning" ||',
    replace: 'path === "/commercial-cleaning-disabled" ||',
    failing: "recognizes both canonical commercial routes",
    why: "Makes the shared site chrome send Edmonton commercial visitors back to the residential price funnel.",
  },
  {
  ...
  },
  {
    ...
    target: "public/api/ghl-quote.php",
    find: "['quote-started', 'quote-left', 'text1-sent']",
    replace: "['quote-left']",
    failing: "a new price check clears the leftover price-check tags first",
    why: "A tag left from an earlier visit blocks GoHighLevel's tag-added trigger, so the next price check gets no card, alert or text.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "const DC_GHL_QUIET_SECONDS = 300;",
    replace: "const DC_GHL_QUIET_SECONDS = 30;",
    failing: "a visit counts as left after five quiet minutes",
    why: "Someone reading a text for a minute would be treated as gone and texted mid-booking.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "json_encode(['tags' => ['quote-left']]",
    replace: "json_encode(['tags' => ['quote-gone']]",
    failing: "a left visit sets the step field, then adds quote-left",
    why: "The office alert and Text 1 never start: the Price-check left workflow triggers on quote-left.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
  ...
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "        'How often' => $line($payload",
    replace: "        'notes' => 'x', 'How often' => $line($payload",
  ...
  },
  {
    ...
    replace: "",
    failing: "a stored submission asks the relay to deliver it now",
    why: "Leads wait up to five minutes for the cron job again, so the office alert and texts lag.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "src/lib/quote-submit.ts",
    find: "...(options.sessionId ? { session_id: options.sessionId } : {}),",
    replace: "",
    failing: "funnel submissions carry the visit id",
    why: "The relay cannot open or close a visit, so nobody is ever detected as left.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "src/lib/quote-presence.ts",
    find: "document.visibilityState === \"visible\" && Date.now() - lastInteraction < PRESENCE_IDLE_MS",
    replace: "Date.now() - lastInteraction < PRESENCE_IDLE_MS",
    failing: "reports only while the page is on screen and recently used",
    why: "A closed or backgrounded tab would keep saying \"still here\" and the visitor would never count as left.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "setPresenceDone(true);",
    replace: "void 0;",
    failing: "reports stop when the visitor is handed to the booking page",
    why: "Reports would keep a confirmed visit looking active; harmless to the relay but wasted requests from every booking.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "'Price shown: ' . $priceShown,",
    replace: "",
    failing: "the left-visit office email names the shown price",
    why: "The office is told to call someone who left without being told which price they saw.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "const DC_GHL_SHOWN_TEXT_KEYS = ['city', ",
    replace: "const DC_GHL_SHOWN_TEXT_KEYS = ['notes', 'city', ",
    failing: "the shown quote is allow-listed: never notes, entry or contact details",
    why: "Customer notes, which can hold lockbox and door codes, would be stored with the visit and mailed to a shared inbox.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "$extra = dc_ghl_shown_fields($config, $session);",
    replace: "$extra = [];",
    failing: "a left visit writes the shown quote to the contact in the step's PUT, before quote-left",
    why: "A VA opening the contact of someone who left sees no price or home, only that they left.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "if ($sealed !== null) $session['shown'] = $sealed;",
    replace: "if ($sealed !== null) { $session['shown'] = $sealed; $session['state'] = 'confirmed'; }",
    failing: "a shown price never makes a lead a confirmation",
    why: "Every price check would count as a confirmed quote, so nobody who left would be flagged and the office would never be told to call.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "public/api/ghl-quote.php",
    find: "$fullName = dc_ghl_name_case(trim($payload['full_name']));",
    replace: "$fullName = trim($payload['full_name']);",
    failing: "shouting names are proper-cased on the way into GoHighLevel",
    why: "Names typed in capitals reach GoHighLevel as \"BROOKE HAYES\" and every follow-up text greets the customer in capitals.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "presenceRef.current?.shown(shownQuoteRef.current);",
    replace: "void 0;",
    failing: "the price screen reports the quote on screen",
    why: "Changing an answer on the price screen never reaches the relay, so the office hears the first price shown, not the last.",
  },
  {
    guard: "src/data/leave-detection.test.ts",
    target: "src/lib/quote-presence.ts",
    find: "export const PRESENCE_MIN_GAP_MS = 30_000;",
    replace: "export const PRESENCE_MIN_GAP_MS = 0;",
    failing: "a changed quote is reported within the relay's request budget, and before the page goes",
    why: "Clicking through extras sends a report every few seconds; the relay's eight-a-minute limit then refuses the confirmation itself.",
  },
  {
    guard: "src/data/quote-funnel-0911.test.ts",
    target: "public/api/ghl-quote.php",
  ...
  },
  {
    guard: "src/data/quote-funnel-0911.test.ts",
    target: "src/data/pricing.ts",
    find: "outlets and vent covers, inside the oven",
    replace: "outlets and vent covers. Already includes deep cleaning. Inside the oven",
  ...
  },
  {
    guard: "src/data/quote-funnel-0911.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "add your address and card",
    replace: "we check your address in this form",
    failing: "QuoteFlow leaves address validation to BookingKoala",
  ...
  // ---- 2026-09-22: travel fee in the price, area question, one main button --
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/data/pricing.ts",
    find: "everything in a standard clean, plus baseboards, doors, switches",
    replace: "handover-day clean, plus baseboards, doors, switches",
    failing: "say what each clean covers, without the rejected lines",
    why: "The move-out card goes back to the handover-day line the owner rejected.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "guideToNext(\"pets\");",
    replace: "shelfRef.current?.scrollIntoView();",
    failing: "move to the next unanswered question, never straight past city limits",
    why: "Answering pets on a phone scrolls past the city-limits question again.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
  ...
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "option.label.match(/^(\\d+)[^(]*(?:",
    replace: "option.label.match(/^(\\d+)\\D*(?:",
    failing: "puts the size cap on every chip, not only in a caption",
    why: "The bedroom sqft cap drops back to a small caption nobody notices.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "`${detailAnswers.filter((answer) => answer.done).length} of ${detailAnswers.length} answered`",
    replace: "`${detailAnswers.length} to go`",
  ...
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "homeTypes.some((option) => option.id === current) ? current : null",
    replace: "homeTypes.some((option) => option.id === current) ? current : homeTypes[0]?.id ?? null",
    failing: "home type has no silent default and is required before the price",
    why: "Apartment is silently pre-chosen again, pricing homes the visitor never described.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "Pick my date &amp; arrival time",
    replace: "Choose my time",
    failing: "the last button says what it opens, with the page switch explained",
    why: "The final button goes back to the vague label that did not stand out.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: 'document.getElementById("email")?.focus();',
    replace: "",
    failing: "Enter on a contact field moves to the next field, and lockbox or code points to the notes",
    why: "Enter on the last-name field stops moving the visitor on to email.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "const next = (index + step + FLEXIBILITY_OPTIONS.length) % FLEXIBILITY_OPTIONS.length;",
    replace: "const next = index;",
    failing: "the flexibility choices take the arrow keys, with one tab stop",
    why: "The arrow keys stop moving the flexibility choice.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "if (first && option.value < 4) guideDetails(\"cleanliness\");",
    replace: "if (first) guideDetails(\"cleanliness\");",
    failing: "the Deep Cleaning suggestion is announced and the page stays with it",
    why: "Rating the home 4 or 5 scrolls the page away from the Deep Cleaning suggestion again.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/index.css",
    find: "  left: 1rem;\n  top: 0.375rem;",
    replace: "  left: 1rem;\n  top: -2.25rem;",
    failing: "the price-change note sits inside the sticky bar",
    why: "The price-change note floats above the bar again, over the page.",
  },
  {
    guard: "src/data/quote-funnel-0923.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: '{ threshold: 0, rootMargin: "0px 0px -33% 0px" }',
  ...
  },
  {
    ...
    replace: "for (const row of basketRows) basket[row.extra.name] = row.quantity;",
    failing: "a home outside city limits carries BookingKoala's travel-fee row into the price and the handoff",
    why: "The price includes the travel fee but the booking page would not tick it.",
  },
  {
    guard: "src/data/quote-funnel-0922.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
  ...
  },
  {
    ...
    why: "Brings back the equal-weight call-back button that competed with Choose my time.",
  },
  {
    guard: "src/data/quote-funnel-0922.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
  ...
  },
  {
    ...
    find: '...presets(edmontonSurrounding, "edmonton", true),',
    replace: '...presets(edmontonSurrounding, "edmonton", false),',
    failing: "a satellite-town page is outside its branch city and names the town",
    why: "Prices a Leduc or St. Albert quote without its travel fee.",
  },
  {
    guard: "src/lib/bk-travel-fee.test.ts",
    target: "../bk-travel-fee.js",
    find: "T3Z: null,",
    replace: "T3Z: 'Calgary',",
    failing: "every Alberta FSA gets the same answer",
    why: "The booking page would drop the fee on Redwood Meadows and Bragg Creek codes the site charges.",
  },
  {
    guard: "src/lib/bk-travel-fee.test.ts",
    target: "../bk-travel-fee.js",
    find: "if (rule.outside !== box.checked) box.click();",
    replace: "if (rule.outside && !box.checked) box.click();",
    failing: "ticks the fee for a nearby town, unticks it in Red Deer, and says why",
    why: "A Red Deer customer who ticked the fee by mistake would keep paying it.",
  },
  {
    guard: "src/lib/bk-travel-fee.test.ts",
    target: "../bk-header-fill.html",
    find: "<!-- Duty Cleaners travel fee by postal code: BEGIN -->",
    replace: "<!-- travel fee block removed -->",
    failing: "ships in the BookingKoala header snippet as its own block",
    why: "The header code pasted into BookingKoala would lack the travel-fee check.",
  },
  {
    guard: "src/data/quote-funnel-0922.test.ts",
    target: "src/data/policy.ts",
    find: "It is not a charge, but on a debit card the amount is set aside until the clean is charged.",
  ...
  // ---- 2026-09-22: price-step rewards, the honest way ---------------------
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/components/quote/FrequencyChips.tsx",
    find: "Full price, no plan",
    replace: "Most popular",
    failing: "has no casino mechanics: no random prices, timers, fake scarcity or unsupported badges",
    why: "Puts back a popularity badge nothing on the site supports (owner, 2026-09-18).",
  },
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "useState<Record<string, number>>(restored?.addOns ?? {})",
    replace: 'useState<Record<string, number>>(restored?.addOns ?? { "Inside Oven": 1 })',
    failing: "starts with every extra unticked",
    why: "Pre-ticks a paid extra: sneaking an item into the basket.",
  },
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: "pricing={planPricing}\n",
    replace: "\n",
    failing: "shows plan savings in dollars from the funnel's own quote",
    why: "The plan cards fall back to percent only, the weaker way to show a saving this size.",
  },
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/components/quote/RollingPrice.tsx",
    find: '<span aria-hidden="true" className="tabular-nums">',
    replace: '<span className="tabular-nums">',
    failing: "keeps rolling digits away from screen readers, which hear the final figure",
    why: "A screen reader would read out every in-between number as the total rolls.",
  },
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/index.css",
    find: "top: -0.75rem;",
    replace: "top: -0.75rem; animation: funnel-float 1700ms ease both;",
    failing: "runs every price-step animation only when motion is welcome",
    why: "Animates for visitors who asked their device for reduced motion.",
  },
  {
    guard: "src/data/price-step-rewards.test.ts",
    target: "src/components/quote/QuoteFlow.tsx",
    find: '"grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_320px]"',
    replace: '"grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]"',
