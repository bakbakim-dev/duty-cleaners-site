...
- The mobile sticky bar has NO button (owner, later 2026-09-23): only the price and a status line
  ("N questions left" → "Extras below (optional)" → "All set"). The only orange button is the page's
  own, at the bottom; the bar hides once it is on screen and while a text field has focus. Main
  buttons are never disabled: with answers missing they mark them, list them above the button and
  focus the first. Guided scrolls stop clear of the bar (scroll-padding on .funnel-canvas).
...
- Lead follow-up in GoHighLevel (owner, 2026-09-23): a price-check lead (tag `quote-started`) gets a
  Sales "New lead" card at once. Leave detection (owner: "move it to when they leave"): while the
  price or details screen is open the funnel reports "still here" every minute (`quote-presence.ts`,
  visit id and step only); after 5 quiet minutes the relay tags the contact `quote-left` and fills
  `funnel_last_step`, and the "Price-check left" workflow emails the office ("call now") and sends one
  text in texting hours if still unconfirmed. A 45-minute safety net adds `quote-left` if detection
  missed it. Then the Quote follow-up sequence (email day 1, text day 2, email day 4, text
